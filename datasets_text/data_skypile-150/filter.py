...
def prepare_city_list():
    tmp_list = open("/home/user/data/data_text/data_skypile-150/china_city_full_list.txt", encoding="utf-8").readlines()
...
def filter(batch, full_city_list):
    return [True if keyword_counting_filtering(text, full_city_list) else False for text in batch["text"] ]


print(os.getpid(), flush=True)
full_city_list = prepare_city_list()
ds = load_from_disk("/home/user/data/data_text/data_skypile-150/SkyPile-150B.hf/train")
ds = ds.filter(filter, batched=True, num_proc=16, fn_kwargs={"full_city_list": full_city_list})
print(len(ds), flush=True)
ds.save_to_disk("/home/user/data/data_text/data_skypile-150/SkyPile-150B.hf/train.filtered", num_proc=16)
