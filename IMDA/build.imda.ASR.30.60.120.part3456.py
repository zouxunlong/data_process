...
def chunking(batch, chunk_limit):
    try:
        ...
        normalized_transcription = [transcription
                                    ...
                                    if transcription[2] > 0 and transcription[3] < array.size/16000]
...
def chunk(split, workers=100):
    ...
    for chunk_limit in [30, 60, 120]:
        if os.path.exists(split.replace(".hf", "_ASR_{}_v1".format(chunk_limit))):
            continue
        ds_built = ds.map(chunking,
                          ...
                          num_proc=workers)
        ds_built.save_to_disk(split.replace(".hf", "_ASR_{}_v1".format(chunk_limit)), num_proc=16)
