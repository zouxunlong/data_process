...
def segment(ds_path, workers=100):

    def segment_batch(batch):
        new_batch = {"context": [], "instruction": [], "answer": [], "other_attributes": []}

        for i in range(len(batch["audio"])):

            if batch["transcription1"][i]:
                for n, transcription in enumerate(batch["transcription1"][i]):
                    start = transcription["start"]
                    end = transcription["end"]
                    sentence = normalize_sentence(transcription["sentence"])
                    segment_array = batch["audio"][i]["array"][max(0, int(start*16000)):max(0, int(end*16000))].copy()
                    if sentence and segment_array.size > 1:
            ...
            if batch["transcription2"][i]:
                for n, transcription in enumerate(batch["transcription2"][i]):
                    start = transcription["start"]
                    end = transcription["end"]
                    sentence = normalize_sentence(transcription["sentence"])
                    segment_array = batch["audio"][i]["array"][max(0, int(start*16000)):max(0, int(end*16000))].copy()
    ...
    features = Features({
        ...
        'other_attributes': {
            "conversation_id": ds["test"].features["conversation_id"],
            "settings": ds["test"].features["settings"],
            "partition": ds["test"].features["partition"],
            "speaker": ds["test"].features["speaker1"],
        }
    })
    ds = ds.map(
        segment_batch,
    ...
    ds = ds.filter(filter, num_proc=workers)
    ds.save_to_disk(ds_path.replace(".hf", "_ASR"), num_proc=10)
