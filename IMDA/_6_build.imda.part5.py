...
def main(workers=20):
    ...
    root = "/scratch/users/astar/ares/zoux/workspaces/data_process/_data_in_processing/imda_raw/PART5"
    speaker_metadata_dict = json.load(open("/scratch/users/astar/ares/zoux/workspaces/data_process/IMDA/speaker_metadata_part5.json"))
    ...
    for wav in sorted(wavs):
        ...
        scripts_files = glob(os.path.join(root, 'Scripts', f'app_{conversation_id}_*_{topic}.TextGrid'), recursive=True)
        ...
        try:
            if abs(wav_time-script_time1) < 0.1 and abs(wav_time-script_time2) < 0.1:
                ...
                print(f"Time not match: {conversation_id}: {script_time1}/{script_time2}/{wav_time}")
        except KeyError as e:
            print(f"Time not match: {e}: {script_time1}/{script_time2}/{wav_time}")
    ...
    save_path = "/scratch/users/astar/ares/zoux/workspaces/data_process/_data_in_processing/IMDA_HF/PART5"
