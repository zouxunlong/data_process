...
#             print(f"{root}/{part}/txt_all/{id}.txt")
#             break
...
#         time_offset = float(open(f"{root}/{part}/txt_all/{id}.txt").readlines()[-1].strip())
...
for part in ["PART3"]:
    root           = "/home/users/astar/ares/zoux/scratch/workspaces/data_process/_data_in_processing/imda/imda_raw"
    textgrid_files = glob(f"{root}/{part}/Scripts_Same/*.TextGrid") + glob(f"{root}/{part}/Scripts_Separate/*.TextGrid")
    txt_files      = glob(f"{root}/{part}/txt_all/*.txt")
    ...
    for textgrid_file in tqdm(textgrid_files):
        ...
        if os.path.exists(textgrid_file.replace("/Scripts_Same/", "/Scripts_fixed/").replace("/Scripts_Separate/", "/Scripts_fixed/")):
            continue
        ...
        # Save the modified TextGrid file
        tg.write(textgrid_file.replace("/Scripts_Same/", "/Scripts_fixed/").replace("/Scripts_Separate/", "/Scripts_fixed/"))
