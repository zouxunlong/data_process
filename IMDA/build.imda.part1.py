...
def fetch_speaker_metadata():
    ...
    speaker_dict = {}
    df1 = pd.read_excel("Part_1_Speaker_Metadata.xlsx", dtype=str)
    df2 = pd.read_excel("Part_2_Speaker_Metadata.xlsx", dtype=str)
...
def process_file(root, script_file, speaker_metadata_dict, ds_dict):
    ...
    for id, script in script_map.items():
        ...
        ds_dict["partition"].append("PART1")

    return ds_dict


def build_hf(
        root="/home/wangbin/workspaces/9_audio_qa_data/ASR_data/imda_speech/IMDA_raw/IMDA_-_National_Speech_Corpus/PART1",
        output_path="PART1.hf",
        workers=5
):
