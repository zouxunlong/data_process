...
def fetch_speaker_metadata():
    ...
    speaker_dict = {}
    with open("/mnt/data/all_datasets/xunlong_working_repo/data_ASR/aishell/resource_aishell/speaker.info") as f:
...
def fetch_transcriptions():
    transcriptions_dict = {}
    with open("/mnt/data/all_datasets/xunlong_working_repo/data_ASR/aishell/data_aishell/transcript/aishell_transcript_v0.8.txt") as f:
        for i, line in enumerate(f):
            id, script = line.split(maxsplit=1)
            transcriptions_dict[id] = "".join(script.split()).strip()
    print('Num of transcriptions = {}'.format(
        len(transcriptions_dict)), flush=True)
    return transcriptions_dict


def process_file(wav_file, speaker_metadata_dict, transcriptions_dict):

    id = wav_file.split("/")[-1].split(".")[0]
    speaker_id=id[7:11]
    transcription = transcriptions_dict[id]

    data = {
        "context": {"audio": wav_file, 
                    "text": None},
        ...
        "other_attributes": {
            ...
            "speaker": {
                "speaker_id": speaker_id,
                "gender": speaker_metadata_dict[speaker_id]["gender"]
                }
        }
    }
    return data
...
def build_hf(
        root="/mnt/data/all_datasets/xunlong_working_repo/data_ASR/aishell",
        output_path="/mnt/data/all_datasets/xunlong_working_repo/data_ASR/aishell/aishell.hf",
):

    speaker_metadata_dict = fetch_speaker_metadata()
    transcriptions_dict = fetch_transcriptions()

    for split in ["test", "dev", "train"]:
        wav_files = glob(os.path.join(root, 'data_aishell/wav', split, '*', '*.wav'), recursive=True)
        wav_files.sort()
        ...
        for wav_file in tqdm(wav_files):
            try:
                data = process_file(wav_file, speaker_metadata_dict, transcriptions_dict)
                ds_dict['context'].append(data['context'])
                ds_dict['instruction'].append(data['instruction'])
                ds_dict['answer'].append(data['answer'])
                ds_dict['other_attributes'].append(data['other_attributes'])

            except Exception as e:
                print(traceback.format_exc(), flush=True)
                print(wav_file, flush=True)
        ...
        print(ds, flush=True)
        ds.save_to_disk(output_path+"/"+split, num_proc=4)
