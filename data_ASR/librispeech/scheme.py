...
def map2schema(ds_path, workers=54):
    ...
    features = Features({
        ...
        'other_attributes': {
            "id": ds["test.clean"].features["id"],
            "chapter_id": ds["test.clean"].features["chapter_id"],
            "speaker_id": ds["test.clean"].features["speaker_id"],
        }
    })
    ds_dict = ds.map(mapping,
                features=features,
                remove_columns=ds["test.clean"].column_names,
                num_proc=workers,
                batch_size=1,
                writer_batch_size=1
                )
    ds_dict=check_samples(ds_dict)
    ds_dict.save_to_disk(ds_path+"_v1", num_proc=4)
    print(f"complete {ds_path}, no error", flush=True)

def check_samples(ds_dict):
    for split, ds in ds_dict.items():
        ...
        for i in range(len(ds)):
            try:
                sample = ds[i]
                if i % 1000 == 0:
                    print('checked {} samples...'.format(i+1), end="\r",flush=True)
            except:
                print(f'Error in {split}: {i}', end="\r", flush=True)
        if problem_index:
            ds = ds.select([i for i in range(len(ds)) if i not in problem_index])
            ds_dict[split]=ds
        else:
