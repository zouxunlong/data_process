...
def build_conversation(dialog):
    speakers=[]
    speaker_mapping=defaultdict(lambda: "<Speaker{}>".format(len(speaker_mapping)+1))
    conversation=""
    for item in dialog:
        sentence = " ".join(re.sub('\s?\[[^\[\]]*\]\s?', " ", item["text"]).split())
        if not sentence:
            continue

        speaker_label=speaker_mapping[item["speaker"]]

        if not speakers:
            conversation += f"{speaker_label}: {sentence}"
        elif speakers[-1] == speaker_label:
            conversation += f" {sentence}"
        else:
            conversation += f"\n{speaker_label}: {sentence}"
...
def chunking(batch, chunk_limit):
    ...
    try:
        ...
        for item in items:
            ...
            elif item["end"] - dialog[0]["start"] < chunk_limit:
                dialog.append(item)
        ...
        for dialog in dialogs:
            start_time   = dialog[0]["start"]
            end_time     = dialog[-1]["end"]
...
def chunk(ds_path, workers=28):
    print("start {}".format(ds_path), flush=True)

    ds = load_from_disk(ds_path)
    for chunk_limit in [30, 60, 120]:
        if os.path.exists(ds_path.replace("/NLB_v1", f"/NLB_ASR_{chunk_limit}_v1")):
            continue
        ds_built = ds.map(chunking,
                          ...
                          num_proc          = workers)
        ds_built.save_to_disk(ds_path.replace("/NLB_v1", f"/NLB_ASR_{chunk_limit}_v1"), num_proc=4)

    print("complete {}".format(ds_path), flush=True)


if __name__ == "__main__":
    chunk("/home/all_datasets/pre_ready_datasets/xunlong_working_repo/data_NLB/train/NLB_v1")
